...
int gdjNTupleToHist_dijet(std::string inConfigFileName)
{
  ...
  const Float_t DRcut = std::stof(config.GetConfigVal("DRPHOTONJET"));
 ...
 //Pt sub bins handling
  ...
  for(ULong64_t entry = 0; entry < nEntries; ++entry){
    ...
    //std::cout << "yj comment: c" << std::endl;
	int multCounter = 0;
	int multCounterGen = 0;
	int multCounterGenMatched = 0;	

    //std::cout << "yj comment: start the jet loop" << std::endl;
    ...
    //std::cout << "yj comment: e" << std::endl;
    ...
    //std::cout << "yj comment: f" << std::endl;
      if(akthi_em_xcalib_jet_pt_p->at(jI) < jtPtBinsLow) continue;
      if(doJetMaxPtCut){
    ...
    //std::cout << "yj comment: g" << std::endl;
    ...
    if(jetSubLeadingIndex==999 || jetLeadingIndex==999) continue; 
