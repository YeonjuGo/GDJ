...
int gdjNtuplePreProc_onlyPhoton(std::string inConfigFileName)
{
  checkMakeDir check;
  ...
  for(auto const & file : fileList){
    ...
    inTree_p->SetBranchStatus("*jet*", 0);
    ...
    //inTree_p->SetBranchStatus("photon_etcone20_ptcorrected", 0);
    //inTree_p->SetBranchStatus("photon_etcone30_ptcorrected", 0);
    //inTree_p->SetBranchStatus("photon_etcone40_ptcorrected", 0);
...
{
  if(argc != 2){
    std::cout << "Usage: ./bin/gdjNtuplePreProc_onlyPhoton.exe <inConfigFileName>" << std::endl;
  ...
  int retVal = 0;
  retVal += gdjNtuplePreProc_onlyPhoton(argv[1]);
  return retVal;
}
