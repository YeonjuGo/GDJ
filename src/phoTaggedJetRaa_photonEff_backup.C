    ...
    for(Int_t cI = 0; cI < nCentBins; ++cI){
        for(Int_t eI = 0; eI < nPhoEtaBins; ++eI){
            ...
            if(isMC){
                ...
                for(Int_t pI = 0; eI < nPhoPtBins+1; ++pI){
                    photon_truthIso_vs_recoIso[cI][eI][pI] = new TH2F(("photon_truthIso_vs_recoIso" + centBinsStr[cI] + "_" + etaBinsStr[eI] + "_" + ptBinsStr[pI] + "_h").c_str(), ";#gamma gen-level isolation [GeV];#gamma reco-level isolation [GeV]", nGammaPtBins, gammaPtBins);
        ...
        for(ULong64_t entry = 0; entry < nEntries; ++entry){
            ...
            for(unsigned int pI = 0; pI < photon_pt_p->size(); ++pI){
                ...
                //     int pdgCode = truth_pdg_p->at(tI);
                //     if(pdgCode  == 13) continue; //muon
                //     if(pdgCode  == 12 || pdgCode  == 14 || pdgCode  == 16 || pdgCode  == 18) continue; //neutrino
                //     
                //     float dR = getDR(truth_eta_p->at(tI),truth_phi_p->at(tI), truthPhotonEta, truthPhotonPhi);
                //    if(dR > phoIsoConeSize) continue; 
                //    TLorentzVector tPart;
                //    tPart.SetPtEtaPhiM(truth_pt_p->at(tI), truth_eta_p->at(tI), truth_phi_p->at(tI), 0.0);

                //    etSum += truth_eta_p->at(tI);
