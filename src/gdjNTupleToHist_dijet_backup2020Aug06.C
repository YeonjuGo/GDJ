...
int gdjNTupleToHist_dijet(std::string inConfigFileName)
{
 ...
 //Pt sub bins handling
  ...
  std::vector<std::vector<Double_t> > gammaCountsPerPtCent_dijet;
  std::vector<std::vector<Double_t> > gammaCountsPerPtCent_dijet_truth;
  for(Int_t pI = 0; pI < nGammaPtBinsSub+1; ++pI){
  ...
  for(ULong64_t entry = 0; entry < nEntries; ++entry){
    ...
    if((isMC && truthPhotonPt > 0) && (getDR(photon_eta_p->at(pI), photon_phi_p->at(pI), truthPhotonEta, truthPhotonPhi) < 0.2)){  
        if(jetTruthSubLeadingIndex==999 || jetTruthLeadingIndex==999) continue; 
