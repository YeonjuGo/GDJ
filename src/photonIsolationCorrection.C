...
//Local
#include "include/photonIsolationCorrection.h"
#include "include/plotUtilities.h"

photonIsolationCorrection::photonIsolationCorrection(std::string inTableFile){SetTable(inTableFile);}

void photonIsolationCorrection::SetTable(std::string inTableFile)
{
  m_tableFileName = inTableFile;

  if(!m_check.checkFileExt(m_tableFileName, "txt")){
    std::cout << "photonIsolationCorrection: Given table \'" << m_tableFileName << "\' is invalid. return isInit=false" << std::endl;
    m_isInit = false;
    return;
  }
  ...
  while(std::getline(inFile, tempStr)){
	...
	if(m_isDescending && m_centVals[m_centVals.size()-1] >= m_centVals[m_centVals.size()-2]){
	  std::cout << "photonIsolationCorrection: Values in table \'" << m_tableFileName << "\' are not descending. return isInit=false" << std::endl;	
	  m_isInit = false;
	  return;
	}
	if(!m_isDescending && m_centVals[m_centVals.size()-1] <= m_centVals[m_centVals.size()-2]){
	  std::cout << "photonIsolationCorrection: Values in table \'" << m_tableFileName << "\' are not ascending. return isInit=false" << std::endl;	
	  m_isInit = false;
	  return;
	}
      }
  ...
  if(m_centVals.size() != 101){
    std::cout << "photonIsolationCorrection: Values in table \'" << m_tableFileName << "\' are not N=101. return isInit=false" << std::endl;	
    m_isInit = false;
    return;
  }
...
double photonIsolationCorrection::GetCent(double inVal)
{
  double outVal = -1;

  if(!m_isInit) std::cout << "photonIsolationCorrection: Initialization failed. GetCent call will return -1" << std::endl;
  else{
...
void photonIsolationCorrection::PrintTableTex()
{
  if(!m_isInit){
    std::cout << "photonIsolationCorrection ERROR - PrintTableTex(): Table is not initialized. return" << std::endl;
    return;
  }
